...
export class MnemonicPage implements OnInit {
  ...
  onNext() {
    ...
    if(!Util.isEmptyObject(this.multType)){
        this.native.Go(this.navCtrl, "/mnemonic-write", {"mult":this.multType,mnemonicStr: this.mnemonicStr, mnemonicList: this.mnemonicList,"totalCopayers":this.multType["totalCopayers"],"requiredCopayers":this.multType["requiredCopayers"],"mnemonicPassword":this.mnemonicPassword,"payPassword":this.payPassword,name:this.name});
        return;
    }
  ...
  saveWalletList(){
    Config.getMasterWalletIdList().push(this.masterWalletId);
            this.localStorage.saveCurMasterId({masterId:this.masterWalletId}).then((data)=>{
              ...
              this.native.Go(this.navCtrl, "/mnemonic-write", {mnemonicStr: this.mnemonicStr, mnemonicList: this.mnemonicList});
            });
  }
