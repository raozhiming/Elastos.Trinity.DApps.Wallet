...
export class TabHomePage implements OnInit {
  ...
  constructor(public navCtrl: NavController, public walletManager: WalletManager,public native:Native,public localStorage:LocalStorage,public zone:NgZone,public events:Events,public popupProvider: PopupProvider){
     //this.init();
  }
  ...
  goPayment() {
    this.localStorage.get('payment').then((val)=>{
      if (val) {
        ...
        this.native.Go(this.navCtrl, "/payment-confirm", JSON.parse(val));
      }
    });
  }
  ...
  onClick(type){
    switch (type){
      case 0:
        this.native.Go(this.navCtrl, "/walletlist");
        break;
      case 1:
      this.native.Go(this.navCtrl, "/coin-list");
        break;
    }
  }

  onItem(item) {
    this.native.Go(this.navCtrl,"/coin", {name: item.name,"elaPer":this.elaPer,"idChainPer":this.idChainPer});
  }
